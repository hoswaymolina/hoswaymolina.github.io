...
We can break this space $S$ up into groups, or *subsets*; for example, let's group together outcomes with a common $n_1$. In other words, the outcomes $`\{n_1 and win\}`$ and $`\{n_1 and loss\}`$ would be in the same subset. Let's call these sets $B_n$
